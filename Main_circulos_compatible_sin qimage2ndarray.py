...
class mainProgram(QMainWindow, Ui_MainWindow):
    ...
    def pasar_a_grises(self):
        ...
        if len(self.contornos) >= 2:
            ...
            (x_centro, y_centro), radio = cv2.minEnclosingCircle(segundo_mayor_contorno)
            x_centro, y_centro, radio = int(x_centro), int(y_centro), int(radio)

            # Crear una máscara circular
            mascara = np.zeros_like(self.imagen_gv2)
            cv2.circle(mascara, (x_centro, y_centro), radio, 255, thickness=-1)

            # Aplicar la máscara
            self.imagen_mascara = cv2.bitwise_and(self.imagen_gv2, self.imagen_gv2, mask=mascara)

            # Recorte del círculo
            x1 = max(0, x_centro - radio)
            y1 = max(0, y_centro - radio)
            x2 = min(self.imagen_mascara.shape[1], x_centro + radio)
            y2 = min(self.imagen_mascara.shape[0], y_centro + radio)
            self.imagen_recortada = self.imagen_mascara[y1:y2, x1:x2]

            # Reescalar a cuadrado
            diametro = 2 * radio
            self.imagen_reescalada = cv2.resize(self.imagen_recortada, (diametro, diametro),
    ...
    def cv2_a_qpixmap(self, imagen):
        if len(imagen.shape) == 2:  # Imagen en escala de grises
            qimage = QtGui.QImage(imagen.data, imagen.shape[1], imagen.shape[0], imagen.strides[0],
                                  QtGui.QImage.Format_Grayscale8)
        else:  # Imagen en color BGR
            imagen_rgb = cv2.cvtColor(imagen, cv2.COLOR_BGR2RGB)
            qimage = QtGui.QImage(imagen_rgb.data, imagen_rgb.shape[1], imagen_rgb.shape[0], imagen_rgb.strides[0],
    ...
    def calcular(self):
        if self.gv2.hasPhoto() and hasattr(self, 'imagen_mascara') and hasattr(self, 'im_th'):
            # Asegurar que la máscara tenga el mismo tamaño que la imagen de umbral
            mascara_binaria = cv2.resize(self.imagen_mascara, (self.im_th.shape[1], self.im_th.shape[0]),
                                         interpolation=cv2.INTER_NEAREST)

            # Crear una máscara binaria: solo los píxeles dentro del círculo/contorno
            mascara_binaria = (mascara_binaria > 0).astype(np.uint8)

            # Aplicar la máscara a la imagen umbralada
            pixeles_dentro = cv2.bitwise_and(self.im_th, self.im_th, mask=mascara_binaria)

            # Contar píxeles blancos dentro del contorno
            n_px_blancos = cv2.countNonZero(pixeles_dentro)

            # Contar solo los píxeles válidos dentro del contorno
            n_px_totales = cv2.countNonZero(mascara_binaria)

            porcentaje = (n_px_blancos / n_px_totales * 100) if n_px_totales > 0 else 0

            self.lb2.setText(f"{porcentaje:.2f} %")
